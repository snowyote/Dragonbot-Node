...
async function updateUser(userID, msg) {
	try{
		let queryRes = await Utils.queryDB("SELECT * FROM users WHERE discordID=" + userID);
		if (queryRes && queryRes.length) {
			var dbUserID = queryRes[0].id;
			var active = queryRes[0].activePet;
			let petRes = await Utils.queryDB("SELECT * FROM pets WHERE id=" + active);
			let userAch = await Utils.queryDB("SELECT achievements FROM users WHERE id=" + dbUserID)
			...
			if(petRes && petRes.length) {
				...
				await Utils.queryDB("UPDATE users SET xp = xp + 1 WHERE discordID = " + userID);
			...
			for (var index = 0; index < achRes.length; index++) {
				...
				if (achProgRes[0][achKeys[varToCheck]] >= varRequired) {
					if (AchJSON.includes(i + 1) == false) {
						...
						msg.channel.send({achUnlock});
						AchJSON.push(i + 1);
						var JSONString = JSON.stringify(AchJSON);
						await Utils.queryDB("UPDATE users SET achievements='" + JSONString + "' WHERE id=" + dbUserID);
						Utils.log("\x1b[36m%s\x1b[0m", "DB: Added achievement " + achievement.id + " to user " + dbUserID);
					}
				}
			}
		} else {
			Utils.log("\x1b[36m%s\x1b[0m", "DB: " + userID + " was NOT found, adding to database!");
			let adduserIDRes = await Utils.queryDB("INSERT INTO users(discordID) VALUES (" + userID + "); ");
			Utils.log("\x1b[36m%s\x1b[0m", "DB: " + userID + " successfully added to database as ID " + adduserIDRes.insertId + "!");
			let addAchievementQuery = Utils.queryDB("INSERT INTO achievement_progress(id) VALUES (" + adduserIDRes.insertId + ")");
